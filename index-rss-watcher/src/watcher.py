...
class FeedWatcher:
    ...
    async def _fetch(self, session: aiohttp.ClientSession) -> bytes:
        ...
        if self.modified:
            hdrs["If-Modified-Since"] = self.modified
        ...
        try:
            async with session.get(url, headers=hdrs, timeout=30) as resp:
                ...
                if resp.status == 304:  # No new content
                    ...
                    log.error(
                        f"Access denied (403) to {url} - check robots.txt compliance"
                    )
        ...
        async with aiohttp.ClientSession(
            ...
            while True:
                try:
                    ...
                    log.exception(
                        f"Fetch error ({consecutive_errors}/{max_errors}): %s", e
                    )

                    if consecutive_errors >= max_errors:
                        log.error(
                            f"Too many consecutive errors ({max_errors}), waiting longer..."
                        )
    ...
    def _process_feed(self, raw: bytes):
        ...
        for entry in feed.entries:
            ...
            if REPLACE_PATTERN.search(description):
                ...
                log.debug(
                    f"Skipping entry (no 'replace' in description): {entry.title}"
                )
